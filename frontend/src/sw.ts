...
// SPA navigation fallback. Every route but `/` exists only client-side, so
// without this a document request for e.g. /members or /photo/:id (a hard
// reload, or a notification click on a cold app) goes to the network and shows
// the browser's own error page when offline — even though the whole shell is
// precached. `index.html` is always in the precache manifest: the build globs
// html, and vite.config.ts sets devOptions.navigateFallback so it is present in
// dev too (createHandlerBoundToURL throws for a non-precached URL).
//
// The denylist is belt-and-braces: NavigationRoute only matches requests with
// mode 'navigate', which API calls never are, but it also keeps the dev proxy's
// /api/* prefix out of the fallback entirely.
registerRoute(
...
// Cache thumbnails with cache-first strategy (small files, ~200KB at most)
// (?:api\/)? prefix handles local dev where vite proxies /api/* to backend
//
// Bounded and quota-aware. An unbounded cache grows for the lifetime of the
// install, and — worse — a cache that doesn't opt into purgeOnQuotaError makes
// the browser evict the caches that *did* (fullsize, below) while thumbnail
// writes keep failing anyway. 200 entries is a few feed-scrolls' worth and caps
// the cache in the tens of megabytes.
registerRoute(
